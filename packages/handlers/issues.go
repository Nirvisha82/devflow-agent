...
func processIssue(ctx *probot.Context, repoName string, issueNumber int, issueTitle string) error {
	...
	aiAnalysis := &ai.IssueAnalysis{
		IssueTitle:       issueTitle,
		IssueDescription: event.Issue.GetBody(),
		Labels:           getIssueLabelNames(event.Issue.Labels),
		RepoStructFile:   analysisFileName,
	}

	result, err := ai.AnalyzeIssueWithAI(aiAnalysis)
	if err != nil {
		slog.Error("Failed to generate AI analysis", "error", err)
		return err
	}

	// Save AI analysis
	aiAnalysisFile := repoPath + "/devflow-agent-IssueAnalysis.md"
	err = repoActions.SaveAnalysisToFile(result.MarkdownContent, aiAnalysisFile)
	if err != nil {
		return err
	}
	...
	err = repoActions.CommitFile(ctx, repoName, IssueProcessing.BranchName,
		"Issue analysis generated by devflow-agent", aiAnalysisFile)
	if err != nil {
		return err
	}
