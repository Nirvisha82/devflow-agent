	...
	if len(result.ChangesMade) > 0 {
		...
		if result.PRBodyFile != "" {
			...
			if err != nil {
				...
				prTitle := fmt.Sprintf("Fix #%d: %s", issueNumber, issueTitle)
				slog.Info("Creating PR with AI-generated body", "length", len(prBodyContent))

				pr, err = repoActions.CreatePullRequest(ctx, repoName, branchName, prTitle, string(prBodyContent))
				if err != nil {
			...
			// No PR body file generated, use default method
			slog.Info("No PR body file returned by agent, using default PR creation")
			pr, err = repoActions.CreateIssueResolutionPR(
				ctx,
				repoName,
				branchName,
